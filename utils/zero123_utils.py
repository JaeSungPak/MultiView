...
def init_model(device, ckpt, half_precision=False):
    ...
    config = OmegaConf.load('./sd-objaverse-finetune-c_concat-256.yaml')
...
@torch.no_grad()
def predict_stage1_gradio(model, raw_im, save_path = "", adjust_set=[], device="cuda", ddim_steps=75, scale=3.0, azim_min=0, azim_max=270):
    ...
    delta_y_1_8 = [azim_min+((azim_max-azim_min)/(adjust_len-1))*i for i in range(adjust_len)]
    print(delta_y_1_8)
    ret_imgs = []
...
@torch.no_grad()
def predict_stage_comb(model, raw_im, save_path = "", device="cuda", ddim_steps=75, scale=3.0, adjust_set=[], elev=[], azim=[]):
    # raw_im = raw_im.resize([256, 256], Image.LANCZOS)
    # input_im_init = preprocess_image(models, raw_im, preprocess=False)
    input_im_init = np.asarray(raw_im, dtype=np.float32) / 255.0
    input_im = transforms.ToTensor()(input_im_init).unsqueeze(0).to(device)
